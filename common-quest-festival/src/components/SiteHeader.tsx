...
export default function SiteHeader({ locale, dict, userEmail, isAdmin, logoUrl }: Props) {
  ...
  return (
    ...
    >
      <div className="shell flex h-32 items-center justify-between gap-4 md:h-44">
        <Link href={`/${locale}`} aria-label="Common Quest, accueil" className="shrink-0">
          <Image
            ...
            height={264}
            priority
            className="h-[76px] w-auto md:h-[104px]"
          />
        </Link>
        ...
        <nav className="hidden items-center gap-7 md:flex" aria-label="Navigation principale">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className={`font-display text-[17px] uppercase tracking-[0.04em] transition-colors hover:text-acid ${
          ...
          {isAdmin && (
            <Link href={`/${locale}/admin`} className="font-display text-[17px] uppercase tracking-[0.04em] text-violet">
              {dict.nav.admin}
            </Link>
          )}
        ...
        >
          <span className="font-display text-[16px] uppercase tracking-[0.04em]">{open ? dict.nav.fermer : dict.nav.menu}</span>
      ...
      {open && (
        <div id="menu-mobile" className="border-t border-white/10 bg-ink md:hidden">
          <div className="shell flex flex-col gap-1 py-5">
            {links.map((l) => (
              <Link key={l.href} href={l.href} className="py-3 font-display text-4xl uppercase">
                {l.label}
              </Link>
            ))}
